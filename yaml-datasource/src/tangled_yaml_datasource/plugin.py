"""
YAML Data Source Plugin implementation.

Parses arbitrary YAML documents into graphs per specification:
- Mappings become nodes; attributes and references become edges
- Supports cyclic references via id attributes and string reference semantics
...
def _to_attribute_value(key: str, value: Any) -> Optional[Any]:
    """
    Convert YAML value to API attribute type (int, float, str, date).
    Returns None for null (skip). Sequences of primitives become comma-separated string.

    YAML safe_load already parses ints, floats, bools, and dates natively,
    but dates may come as datetime.date or datetime.datetime objects.
    """
    if value is None:
        return None
...
def _collect_objects(
    ...
    """Recursively collect all YAML mappings. Uses id(obj) to deduplicate."""
...
def _collect_from_root(data: Any) -> List[Dict[str, Any]]:
    """Collect all mappings from root (mapping or sequence of mappings)."""
    objects: List[Dict[str, Any]] = []
    obj_to_path: Dict[int, str] = {}
...
class YamlDataSource(DataSourcePlugin):
    """
    Data source plugin that parses YAML files into graphs.

    Mapping rules (per specification):
    - YAML mappings become nodes
    - Nested mappings/sequences create edges to child nodes
    ...
    def description(self) -> str:
        return "Parses YAML files and constructs graphs. Supports cyclic references via id attributes."
    ...
    def load(self, **params: Any) -> Graph:
        """
        Parse YAML file and construct a graph.
        ...
        Returns:
            Graph constructed from YAML data

        Raises:
            ValueError: If file_path missing, file not found, or invalid YAML
        """
        ...
        if not isinstance(data, (dict, list)):
            raise ValueError("YAML root must be a mapping or sequence")
